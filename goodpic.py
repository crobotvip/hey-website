...
while True:
    if flag==1:
        ...
        for a in b:
            ...
            tittle=re.findall(r'<h5>(.*)</h5>',str(getpage.content))
            tittle=['xxxx','yyyy']
            pages=[]
            threads=[]
            #pages=re.findall(r'<a href="(.*?)" class="page-en">(.*?)</a>',str(getpage.content))
            #page=pages[0]
        ...
        if get.status_code==200:
            ...
            for a in b:
                ...
                pages=[]
                threads=[]
                pages=re.findall(r'<span class="page-ch">共(.*?)页</span>',str(getpage.content))
                page=pages[0]
                download_url='http://img1.mm131.me/pic/'+str(a)+'/'
                for t in 10:
                    if os.path.exists(t)==False:
